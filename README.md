...
이 MVP는 한 명의 기본 작성자를 전제로 합니다. `samples/`에 작성자의 기존 글을 넣고 `node ./bin/blog-writer.js profile`을 실행하면 `writer-style/` 아래에 작성자 스타일 파일이 생성됩니다. 이후 `draft` 명령은 이 스타일 파일을 참고해 새 원고를 정리합니다.
...
```bash
node ./bin/blog-writer.js draft inputs/draft.md --slug my-post
```
...
`npm link`를 실행한 뒤에는 같은 명령을 짧게 쓸 수 있습니다.
